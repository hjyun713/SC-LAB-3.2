...
class JDETracker(object):
    def __init__(self, opt, frame_rate=30):
        self.opt = opt
        self.model = Darknet(opt.cfg, nID=0)

        # load_darknet_weights(self.model, opt.weights)
        self.model.load_state_dict(torch.load(opt.weights, map_location='cuda:0')['model'], strict=False)
        # self.model.load_state_dict(torch.jit.load(opt.weights,map_location='cuda:0')['model'],strict=False)
